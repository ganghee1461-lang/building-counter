export async function onRequestGet(context) {
  ...
  // domain 파라미터 명시적으로 추가
  const vworldUrl = `https://api.vworld.kr/req/wfs?key=${VWORLD_KEY}&domain=building-counter.pages.dev&service=WFS&version=2.0.0&request=GetFeature&typename=${typename}&bbox=${minX},${minY},${maxX},${maxY},EPSG:4326&maxFeatures=${maxFeatures}&output=application/json&srsname=EPSG:4326`;
  ...
  try {
    ...
    if (text.trim().startsWith('<')) {
      return json({ error: 'XML 오류 응답', raw: text.substring(0, 500) }, 502);
    }
